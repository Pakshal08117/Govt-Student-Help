 ...
 * 
 * This file contains location data aligned with the official administrative
 * hierarchy of India. For complete hierarchy information, see indiaAdminHierarchy.ts
 * 
 ...
 * 5. Districts: ~806 (dynamic)
 * 6. Sub-districts (Tehsil/Taluka/Mandal): ~7,800+
 * 7. Development Blocks: Several thousand
...
/**
 * Get the total count of states
 */
export function getTotalStates(): number {
  return INDIA_STATS.TOTAL_STATES;
}

/**
 * Get the total count of Union Territories
 */
export function getTotalUTs(): number {
  return INDIA_STATS.TOTAL_UTS;
}

/**
 * Get the total count of States + UTs
 */
export function getTotalStatesAndUTs(): number {
  return INDIA_STATS.TOTAL_STATES_UTS;
}

/**
 * Check if a location is a State or Union Territory
 */
export function isUnionTerritory(name: string): boolean {
  ...
  return unionTerritories.includes(name);
}

/**
 * Get the appropriate sub-district term for a state
 * Returns: Tehsil, Taluka, Mandal, or Sub-division
 */
export function getSubDistrictTerm(stateName: string): string {
  return getSubDistrictTermForState(stateName);
}
...
export function getAdminStats() {
  return {
    states: INDIA_STATS.TOTAL_STATES,
    unionTerritories: INDIA_STATS.TOTAL_UTS,
    total: INDIA_STATS.TOTAL_STATES_UTS,
    districts: INDIA_STATS.APPROX_DISTRICTS,
    subDistricts: INDIA_STATS.APPROX_SUB_DISTRICTS,
    gramPanchayats: INDIA_STATS.APPROX_GRAM_PANCHAYATS,
    villages: INDIA_STATS.APPROX_VILLAGES
  };
}
